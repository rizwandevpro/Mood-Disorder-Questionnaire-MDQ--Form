...
// ─────────────────────────────────────────────────────────────────────────────
// GAD7Form.js — GAD-7 Anxiety Screener UI
// Auto-advances on question selection. Shows score summary before submit.
// ─────────────────────────────────────────────────────────────────────────────

import { useState, useEffect, useRef, useCallback } from "react";
import { STEPS, QUESTIONS, OPTIONS, DIFFICULTY_OPTIONS, TOTAL_STEPS, THANKYOU_STEP } from "./gad7Steps";
...
const GAD_CSS = `
  @keyframes gadSlideInRight { from{opacity:0;transform:translateX(60px) scale(0.97)} to{opacity:1;transform:translateX(0) scale(1)} }
  @keyframes gadSlideInLeft  { from{opacity:0;transform:translateX(-60px) scale(0.97)} to{opacity:1;transform:translateX(0) scale(1)} }
  @keyframes gadSlideOutLeft { from{opacity:1;transform:translateX(0) scale(1)} to{opacity:0;transform:translateX(-60px) scale(0.97)} }
  @keyframes gadSlideOutRight{ from{opacity:1;transform:translateX(0) scale(1)} to{opacity:0;transform:translateX(60px) scale(0.97)} }
  .gad-enter-right{ animation: gadSlideInRight  0.38s cubic-bezier(0.22,1,0.36,1) forwards; }
  .gad-enter-left { animation: gadSlideInLeft   0.38s cubic-bezier(0.22,1,0.36,1) forwards; }
  .gad-exit-left  { animation: gadSlideOutLeft  0.22s cubic-bezier(0.4,0,1,1) forwards; pointer-events:none; }
  .gad-exit-right { animation: gadSlideOutRight 0.22s cubic-bezier(0.4,0,1,1) forwards; pointer-events:none; }
  .gad-option:hover { border-color: #7d4f50 !important; }
`;
if (typeof document !== "undefined" && !document.getElementById("gad-styles")) {
  const t = document.createElement("style");
  t.id = "gad-styles";
  t.textContent = GAD_CSS;
...
function scoreLabel(score) {
  if (score <= 4)  return { label: "Minimal Anxiety",  color: "#16a34a" };
  if (score <= 9)  return { label: "Mild Anxiety",     color: "#ca8a04" };
  if (score <= 14) return { label: "Moderate Anxiety", color: "#ea580c" };
  return                  { label: "Severe Anxiety",   color: "#dc2626" };
}
  ...
  return (
    <div style={{backgroundColor:"white",borderRadius:"24px",boxShadow:"0 4px 24px rgba(0,0,0,0.08)",border:"1px solid #f1f5f9",overflow:"hidden"}}>
      <div style={{padding:"32px",borderBottom:"1px solid #f1f5f9",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"24px",fontWeight:700,color:"#0f172a",marginBottom:"8px",fontFamily:"'Lora', Georgia, serif"}}>GAD-7 Anxiety Screener</h2>
        <p style={{fontSize:"15px",color:"#64748b",lineHeight:1.6,fontFamily:"'Source Sans 3', sans-serif",maxWidth:"420px",margin:"0 auto"}}>
          Over the <strong style={{color:"#1e293b"}}>last two weeks</strong>, how often have you been bothered by the following problems?
        </p>
      </div>
      <div style={{padding:"28px 32px",backgroundColor:"#f8fafc"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"10px",marginBottom:"20px"}}>
          {OPTIONS.map(o => (
            <div key={o.label} style={{textAlign:"center",padding:"12px 8px",borderRadius:"12px",border:"2px solid #e2e8f0",backgroundColor:"white"}}>
              <div style={{fontSize:"22px",fontWeight:800,color:BRAND,fontFamily:"'Lora', serif"}}>{o.score}</div>
              <div style={{fontSize:"11px",fontWeight:600,color:"#64748b",marginTop:"4px",fontFamily:"'Source Sans 3', sans-serif",lineHeight:1.3}}>{o.label}</div>
            </div>
          ))}
        </div>
        <p style={{fontSize:"12px",color:"#94a3b8",textAlign:"center",fontFamily:"'Source Sans 3', sans-serif"}}>7 questions • auto-advances on selection • takes ~1 minute</p>
      </div>
...
// ── Step: question ────────────────────────────────────────────────────────────
function StepQuestion({ step, answers, onChange, onBack, questionNumber, totalQuestions }) {
    ...
    <div style={{backgroundColor:"white",borderRadius:"24px",boxShadow:"0 4px 24px rgba(0,0,0,0.08)",border:"1px solid #f1f5f9",overflow:"hidden"}}>
      ...
      <div style={{padding:"24px 32px 20px",borderBottom:"1px solid #f1f5f9"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px"}}>
          ...
          <span style={{fontSize:"11px",fontWeight:600,color:BRAND,fontFamily:"'Source Sans 3', sans-serif"}}>
            Over the last two weeks…
          </span>
        </div>
        <p style={{fontSize:"19px",fontWeight:600,color:"#0f172a",lineHeight:1.4,fontFamily:"'Lora', Georgia, serif"}}>
          {step.text}
        </p>
      </div>
            ...
            <button
              ...
              <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
                <div style={{
                  ...
                  border:`2px solid ${isSelected?BRAND:"#cbd5e1"}`,
              ...
              <span style={{
                ...
                backgroundColor:isSelected?BRAND:"#e2e8f0",
      ...
      {/* Footer */}
      <div style={{padding:"16px 28px 20px",display:"flex",justifyContent:"flex-start"}}>
...
function StepDifficulty({ answers, onChange, onNext, onBack }) {
  ...
  const { label: severityLabel, color: severityColor } = scoreLabel(totalScore);

  // Column totals
  const colTotals = OPTIONS.map(opt =>
    QUESTIONS.filter(q => answers[q.key] === opt.label).length
  );
    ...
    <div style={{backgroundColor:"white",borderRadius:"24px",boxShadow:"0 4px 24px rgba(0,0,0,0.08)",border:"1px solid #f1f5f9",overflow:"hidden"}}>
      ...
      <div style={{padding:"24px 32px",borderBottom:"1px solid #f1f5f9",backgroundColor:"#fafaf9"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"20px",flexWrap:"wrap"}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:"42px",fontWeight:800,color:severityColor,fontFamily:"'Lora', serif",lineHeight:1}}>{totalScore}</div>
            <div style={{fontSize:"11px",fontWeight:600,color:"#94a3b8",fontFamily:"'Source Sans 3', sans-serif",marginTop:"2px"}}>Total / 21</div>
          </div>
          <div style={{flex:1}}>
            <span style={{display:"inline-block",padding:"6px 16px",borderRadius:"20px",fontSize:"14px",fontWeight:700,backgroundColor:severityColor+"18",color:severityColor,fontFamily:"'Source Sans 3', sans-serif"}}>
              {severityLabel}
            </span>
            <div style={{display:"flex",gap:"8px",marginTop:"10px",flexWrap:"wrap"}}>
              {OPTIONS.map((opt, i) => (
                <div key={opt.label} style={{textAlign:"center",padding:"6px 10px",borderRadius:"8px",border:"1px solid #e2e8f0",backgroundColor:"white",minWidth:"50px"}}>
                  <div style={{fontSize:"16px",fontWeight:700,color:BRAND,fontFamily:"'Lora', serif"}}>{colTotals[i]}</div>
                  <div style={{fontSize:"9px",fontWeight:600,color:"#94a3b8",fontFamily:"'Source Sans 3', sans-serif",lineHeight:1.2,marginTop:"2px"}}>{opt.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {DIFFICULTY_OPTIONS.map(opt => {
            ...
            return (
              <button key={opt} type="button" onClick={()=>onChange("difficulty",opt)}
                ...
                <div style={{width:"18px",height:"18px",borderRadius:"50%",flexShrink:0,border:`2px solid ${isSel?BRAND:"#cbd5e1"}`,backgroundColor:isSel?BRAND:"white",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}>
                  {isSel && <div style={{width:"7px",height:"7px",borderRadius:"50%",backgroundColor:"white"}} />}
                </div>
                <span style={{fontSize:"14px",fontWeight:isSel?600:500,color:isSel?BRAND:"#374151",fontFamily:"'Source Sans 3', sans-serif"}}>
                  {opt}
                </span>
              </button>
            );
      ...
      <div style={{padding:"20px 32px",backgroundColor:"#f8fafc",borderTop:"1px solid #f1f5f9",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"16px"}}>
        ...
        <button type="button" onClick={onNext} disabled={!selected}
          style={{display:"flex",alignItems:"center",gap:"8px",padding:"12px 28px",borderRadius:"12px",fontSize:"14px",fontWeight:600,border:"none",color:"white",backgroundColor:selected?BRAND:"#cbd5e1",cursor:selected?"pointer":"not-allowed",boxShadow:selected?BRAND_SHADOW:"none",fontFamily:"'Source Sans 3', sans-serif",transition:"background-color 0.2s"}}>
          Submit
...
function AnimatedCard({ direction, children }) {
  ...
  const cls = direction === "back" ? "gad-enter-left" : "gad-enter-right";
  return (
...
// ── Main export ───────────────────────────────────────────────────────────────
export default function GAD7Form({ currentStep, answers, onChange, onNext, onBack }) {
  ...
  // Respond to parent step changes
  useEffect(() => {
  ...
  const exitClass = direction === "back" ? "gad-exit-right" : "gad-exit-left";
  const step      = STEPS[displayStep] || STEPS[currentStep];

  // Question number among question-type steps
  ...
  const renderStep = () => {
    ...
    switch (step.type) {
      case "info":
        return <StepInfo onNext={handleNext} />;
      case "question":
        return (
          <StepQuestion
            step={step}
